...
from light_training.utils.lr_scheduler import get_constant_schedule_with_warmup, get_cosine_schedule_with_warmup,\
...
class Trainer:
    def __init__(self, env_type,
        ...
        self.num_step_per_epoch = 150 // self.num_gpus
        ...
        if num_gpus > gpu_count:
            print("Mismatch in the number of GPUs")
            os._exit(0)
    ...
    def validation_single_gpu(self, val_dataset, ):
        ...
        print("Average Dice: SL".format(v_sum[0]))
        print("Average Dice: {}  ".format(((v_sum[0]) / len(v_sum))))
        return v_sum, val_outputs
    def validation_single_gpu_two_metrics(self, val_dataset, ):
        if self.ddp:
            print(f"single gpu model not support the ddp")
            exit(0)
        val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False, pin_memory=True)
        ...
        for idx, batch in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            batch = self.to_device(batch)
            with torch.no_grad():
                dice_out, hd95_out = self.validation_step(batch)
                assert dice_out is not None
            return_list = False
            dice_outputs.append(dice_out)
            hd95_outputs.append(hd95_out)
        if isinstance(dice_out, list) or isinstance(dice_out, tuple) or isinstance(hd95_out, list) or isinstance(
                hd95_out, tuple):
            return_list = True
        dice_outputs = torch.tensor(dice_outputs)
        hd95_outputs = torch.tensor(hd95_outputs)
        if not return_list:
            length = 0
            dice_sum = 0.0
            for v in dice_outputs:
                if not torch.isnan(v):
                    dice_sum += v
                    length += 1
            hd95_sum = 0.0
            for v in hd95_outputs:
                if not torch.isnan(v):
                    hd95_sum += v
            if length == 0:
                dice_sum = 0
                ...
                dice_sum = dice_sum / length
                hd95_sum = hd95_sum / length
        else:
            num_val = len(dice_outputs[0])
            length = [0.0 for i in range(num_val)]
            dice_sum = [0.0 for i in range(num_val)]
            hd95_sum = [0.0 for i in range(num_val)]
            for v in dice_outputs:
                for i in range(num_val):
                    if not torch.isnan(v[i]):
                        dice_sum[i] += v[i]
            ...
            for i in range(num_val):
                if length[i] == 0:
                    ...
                    dice_sum[i] = dice_sum[i] / length[i]
                    hd95_sum[i] = hd95_sum[i] / length[i]
        print("Average Dice: SL: {}".format(dice_sum[0]))
        print("Average HD95: SL: {}".format(hd95_sum[0]))
        return dice_sum, dice_outputs
    def validate(self):
        val_outputs = []
        if self.global_step % self.val_every == 0\
            ...
            for i in tqdm(range(len(self.val_loader)), total=len(self.val_loader)):
                ...
                batch = self.to_device(batch)
                with torch.no_grad():
                    with torch.autocast("cuda", enabled=True) if (
                            self.ddp or 'cuda' in self.device) else dummy_context():
                        val_out = self.validation_step(batch)
                        assert val_out is not None
            ...
            if self.ddp:
                ...
                for i in range(len(outputs_split)):
                    ...
                    val_outputs_merge.append(
                        distributed_concat(val_outputs, num_total_examples=len(self.val_loader) * self.num_gpus))
            else:
                val_outputs_merge = []
                for i in range(len(outputs_split)):
