...
class IP_MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        m = x.reshape(B, H // self.segment_dim, self.segment_dim, W // self.segment_dim, self.segment_dim, D,
                      C).permute(0, 1, 3, 5, 6, 2, 4).reshape(B, H // self.segment_dim, W // self.segment_dim, D, C,
                                                              self.segment_dim * self.segment_dim)
        m = self.attention_reweight(m).reshape(B, H // self.segment_dim, W // self.segment_dim, D, C, self.segment_dim,
                                               self.segment_dim).permute(0, 1, 5, 2, 6, 3, 4).reshape(B, H, W, D, C)
        h = x.transpose(2, 1).reshape(B, H * W // self.segment_dim, self.segment_dim, D, self.segment_dim, S).permute(0,
                                                                                                                      1,
                                                                                                                      4,
                                                                                                                      3,
                                                                                                                      2,
                                                                                                                      5).reshape(
            B, H * W // self.segment_dim, self.segment_dim, D,
            self.segment_dim * S)
        h = self.mlp_h(h).reshape(B, H * W // self.segment_dim, self.segment_dim, D, self.segment_dim, S).permute(0, 1,
                                                                                                                  4, 3,
                                                                                                                  2,
                                                                                                                  5).reshape(
            B, W, H, D, C).transpose(2, 1)
        w = x.reshape(B, H * W // self.segment_dim, self.segment_dim, D, self.segment_dim, S).permute(0, 1, 4, 3, 2,
                                                                                                      5).reshape(B,
                                                                                                                 H * W // self.segment_dim,
                                                                                                                 self.segment_dim,
                                                                                                                 D,
                                                                                                                 self.segment_dim * S)
        w = self.mlp_w(w).reshape(B, H * W // self.segment_dim, self.segment_dim, D, self.segment_dim, S).permute(0, 1,
                                                                                                                  4, 3,
                                                                                                                  2,
                                                                                                                  5).reshape(
            B, H, W, D, C)
...
class TP_MLP(nn.Module):
    ...
    def forward(self, x):
        B, H, W, D, C = x.shape
        S = C // self.segment_dim
        d = x.reshape(B, H, W, D // self.segment_dim, self.segment_dim, self.segment_dim, S).permute(0, 1, 2, 3, 5, 4,
                                                                                                     6).reshape(B, H, W,
                                                                                                                D // self.segment_dim,
                                                                                                                self.segment_dim,
                                                                                                                self.segment_dim * S)
        x = self.mlp_d(d).reshape(B, H, W, D // self.segment_dim, self.segment_dim, self.segment_dim, S).permute(0, 1,
                                                                                                                 2, 3,
                                                                                                                 5, 4,
                                                                                                                 6).reshape(
            B, H, W, D, C)
