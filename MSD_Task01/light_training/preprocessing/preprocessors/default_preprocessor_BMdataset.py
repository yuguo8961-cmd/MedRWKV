...
class DefaultPreprocessor(object):
    def __init__(self, 
        ...
        self.ntth_data_dir = "NTTH-Dataset"
        self.fuscc_data_dir = "FUSCC-Dataset"

        self.BM_NII = "BM-NII"
        self.BM_NII_mask = "BM-NII-Mask"
        self.Non_BM_NII = "Non-BM-NII"
        self.Non_BM_NII_mask = "Non-BM-NII-Mask"

        self.ntth_BM_NII = "BM-NII"
        self.ntth_mask = "Mask"
    ...
    def get_iterable_list(self):
        fuscc_cases = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.BM_NII,  f"*/*.nii.gz")))
        fuscc_cases_mask = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.BM_NII_mask,  f"*/*.nii.gz")))
        
                                                
                                                          
                 
        fuscc_cases_nonbm = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.Non_BM_NII,  f"*/*.nii.gz")))
        fuscc_cases_nonbm_mask = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.Non_BM_NII_mask,  f"*/*.nii.gz")))
                                                            
                                                                      
                 
        fuscc_cases_2 = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.BM_NII,  f"*.nii.gz")))
        fuscc_cases_mask_2 = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.BM_NII_mask,  f"*.nii.gz")))
        print(f"fuscc_cases_2 is {fuscc_cases_2}")
        print(f"fuscc_cases_mask_2 is {fuscc_cases_mask_2}")
        
        fuscc_cases_nonbm_2 = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.Non_BM_NII,  f"*.nii.gz")))
        fuscc_cases_nonbm_mask_2 = sorted(glob.glob(os.path.join(self.base_dir, self.fuscc_data_dir, self.Non_BM_NII_mask,  f"*.nii.gz")))
        print(f"fuscc_cases_nonbm_2 is {fuscc_cases_nonbm_2}")
        print(f"fuscc_cases_nonbm_mask_2 is {fuscc_cases_nonbm_mask_2}")
        
        ntth_cases = sorted(glob.glob(os.path.join(self.base_dir, self.ntth_data_dir, self.ntth_BM_NII + "*.nii.gz")))
        ntth_cases_mask = sorted(glob.glob(os.path.join(self.base_dir, self.ntth_data_dir, self.ntth_mask + "*.nii.gz")))
        
        print(f"ntth_cases is {ntth_cases}")
        print(f"ntth_cases_mask is {ntth_cases_mask}")
        all_cases = fuscc_cases + fuscc_cases_nonbm + ntth_cases + fuscc_cases_2 + fuscc_cases_nonbm_2
        all_cases_mask = fuscc_cases_mask + fuscc_cases_nonbm_mask + ntth_cases_mask + fuscc_cases_mask_2 + fuscc_cases_nonbm_mask_2

        return all_cases, all_cases_mask
    ...
    def read_data(self, case_name, mask_name):
                              
        try:
            data = sitk.ReadImage(case_name)
        except:
    ...
    def run_case(self, case_name, mask_name):
        """"""
        data, seg, properties = self.read_data(case_name, mask_name)
        if data is not None:
    ...
    def run_case_save(self, case_name, mask_name):
        print(case_name + "~~~~~~~~" * 10)
        data, seg, properties = self.run_case(case_name, mask_name)
        if data is not None:
    ...
    def experiment_plan(self, case_name, mask_name):
    ...
    def run_plan(self):
        all_cases, all_cases_mask = self.get_iterable_list()
        spacings = []
        sizes = []
        intensities_per_channels = []
        print(f"analysing data......")
        for case, mask in tqdm(zip(all_cases, all_cases_mask), total=len(all_cases)):
            spacing, size, intensities_per_channel = self.experiment_plan(case, mask)
            if spacing is None:
                continue
    ...
    def run(self, output_spacing, 
        ...
        for case_name, mask_name in zip(all_cases, all_cases_mask):
            self.run_case_save(case_name, mask_name)
            break
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes) as p:
            for case_name, mask_name in zip(all_cases, all_cases_mask):
                r.append(p.starmap_async(self.run_case_save,
                                         ((case_name, mask_name),)))
            remaining = list(range(len(all_cases)))
                                                                                            
                                                               
            workers = [j for j in p._pool]
            with tqdm(desc=None, total=len(all_cases)) as pbar:
