...
class MultiModalityPreprocessor(object):
    ...
    def run_case(self, case_name):
        ...
        data, data_global, seg = self.run_case_npy(data, seg, properties)

        return data, data_global, seg, properties 

    def run_case_save(self, case_name):
        print(case_name + "~~~~~~~~" * 10)
        data, data_global, seg, properties = self.run_case(case_name)
                                                
        case_name = case_name.split(".")[0]
        np.savez_compressed(os.path.join(self.output_dir, case_name) + '.npz', data=data, data_global=data_global, seg=seg)
