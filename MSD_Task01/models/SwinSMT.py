...
class SwinSMT(nn.Module):
    ...
    def __init__(
        ...
        self.swinViT = SwinTransformer(
            ...
            use_v2=use_v2,
            use_moe=use_moe,
            num_layers_with_moe=num_layers_with_moe,
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_moe:
            self.mlp = DynamicSlotsSoftMoE(
                dim=dim, expert_mult=mlp_ratio, num_experts=num_experts
            )
        else:
            self.mlp = Mlp(
                hidden_size=dim,
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                use_checkpoint=use_checkpoint,
                use_moe=use_moe
                if (self.num_layers - i_layer) <= num_layers_with_moe
                else False,
                num_experts=num_experts,
            )
            if i_layer == 0:
