...
os.environ['PATH'] = '/home/anaconda3/envs/XXX/bin:' + os.environ.get('PATH', '')
T_MAX = 512*512
from torch.utils.cpp_extension import load
wkv_cuda = load(name="wkv", sources=[".models/wkv_op.cpp", ".models/wkv_cuda.cu"],
...
class Z_RiR(nn.Module):
    def __init__(self,
        ...
        self.RiR_backbone = PyramidRiR_enc(img_size=(128, 128), outer_dims=channels, in_chans=in_channels)
...
if __name__ == "__main__":
    ...
    model = Z_RiR(in_channels=4,
                        out_channels=4,)
    x = torch.randn(1, 4, 121, 128, 128).to(device)
    out = model(x)
