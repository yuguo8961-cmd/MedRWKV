...
class BraTSTrainer(Trainer):
    ...
    def training_step(self, batch):
        ...
        if model.training and model.deep_supervision:
            ...
            label_aux1 = F.interpolate(
                label.unsqueeze(1).float(),
                size=pred_aux1.shape[2:],
                mode='nearest'
            ).squeeze(1).long()
            label_aux2 = F.interpolate(
                label.unsqueeze(1).float(),
                size=pred_aux2.shape[2:],
                mode='nearest'
            ).squeeze(1).long()
            label_aux3 = F.interpolate(
                label.unsqueeze(1).float(),
                size=pred_aux3.shape[2:],
                mode='nearest'
            ).squeeze(1).long()
            loss = self.loss_func(pred_main, label) + self.loss_dice(pred_main, label, softmax=True)
            loss += (self.loss_func(pred_aux1, label_aux1) + self.loss_dice(pred_aux1, label_aux1, softmax=True))
            loss += (self.loss_func(pred_aux2, label_aux2) + self.loss_dice(pred_aux2, label_aux2, softmax=True))
            loss += (self.loss_func(pred_aux3, label_aux3) + self.loss_dice(pred_aux3, label_aux3, softmax=True))
            loss = loss / 4.0
        else:
            pred_main = outputs
            loss = self.loss_func(pred_main, label) + self.loss_dice(pred_main, label, softmax=True)
...
parameter_save_path = "./data/3D_parameter_BraTS2023_model"
parameter_name = "MedRWKV_BraTS_2023.pth"
env = "DDP"
